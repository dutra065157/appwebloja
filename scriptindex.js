    // Classe Carrinho (compatível com produtos.html)
        class Carrinho {
            ...
            mostrarFeedback(produto) {
                ...
                feedback.className = 'cart-feedback fixed top-4 right-4 z-50 p-4 rounded-xl shadow-lg transform translate-x-full transition-transform duration-300 bg-gradient-to-r from-green-500 to-emerald-600 text-white';
                feedback.innerHTML = `
        ...
        function setupLoginButtons() {
            ...
            const userButtons = document.querySelectorAll('button[onclick*="login.html"]');
        ...
        function inicializarNewsletter() {
            ...
            if (newsletterForm) {
                newsletterForm.addEventListener('submit', function(e) {
                    ...
                    // Simular envio
                    setTimeout(() => {
                        mostrarNotificacao(`Obrigado por assinar nossa newsletter! Enviaremos novidades para: ${email}`, 'success');
                        emailInput.value = '';
        ...
        function mostrarNotificacao(mensagem, tipo) {
            ...
            notificacao.innerHTML = `
                <div class="flex items-center">
                    <div class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center mr-3">
                        <i data-feather="${tipo === 'success' ? 'check' : tipo === 'error' ? 'alert-triangle' : 'info'}" class="w-5 h-5"></i>
                    </div>
                    <div>
        ...
        async function carregarProdutosEmDestaque(forcarCache = false) {
            ...
            try {
                ...
                const novos = todosProdutos.filter(p => p.novo && !maisVendidos.some(mv => mv.id === p.id));
                const outros = todosProdutos
                    .filter(p => !maisVendidos.some(mv => mv.id === p.id) && !novos.some(n => n.id === p.id));
        ...
        function renderizarProdutosDestaque(produtos) {
            ...
            container.innerHTML = produtos.map((produto, index) => {
                ...
                const imagemSrc = temImagem 
                    ? (API_BASE_URL ? `${API_BASE_URL}${produto.imagem_url}` : produto.imagem_url) 
                    : '';
                ...
                <div class="product-card group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 border border-gray-100 animate-fade-in-up"
                    ...
                    <div class="relative overflow-hidden">
                        ...
                        <div class="w-full h-48 overflow-hidden bg-gray-100">
                            ${temImagem ? 
                                ...
                                `<div class="w-full h-full ${produto.cor_gradiente || 'from-gray-400 to-gray-600'} bg-gradient-to-br flex items-center justify-center group-hover:scale-105 transition-transform duration-500">
                                    <i data-feather="${produto.icone || 'gift'}" class="w-16 h-16 text-white opacity-75"></i>
                                </div>`
                            }
                        </div>
                    ...
                    <div class="p-4">
                        <div class="flex justify-between items-start mb-3">
                            ...
                            <span class="bg-${produto.cor || 'gray'}-100 text-${produto.cor || 'gray'}-600 text-xs px-2 py-1 rounded-full whitespace-nowrap flex-shrink-0">
                                ${produto.categoria}
                            </span>
                        </div>
                        
                        <p class="text-gray-500 text-sm mb-4 line-clamp-2 leading-relaxed">
                            ${produto.descricao || 'Produto especial da Graça Presentes'}
                        </p>
                        ...
                        <div class="flex justify-between items-center">
                            <div class="flex flex-col">
                                ...
                                ${produto.preco_original && parseFloat(produto.preco_original) > precoFinal ? 
                                    `<span class="text-xs text-gray-400 line-through">
                                        De: R$ ${parseFloat(produto.preco_original).toFixed(2).replace('.', ',')}
                                    </span>` : 
                                    ''}
                            </div>
        ...
        document.addEventListener('visibilitychange', function() {
            if (document.visibilityState === 'visible') {
                ...
                if (!lastLoad || (now - parseInt(lastLoad)) > 60000) { // 1 minuto
                    carregarProdutosEmDestaque(true);
